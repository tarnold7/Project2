...
var actorChars = {
  ...
  "o": Coin,
  "*": Flower,
  ">": SpeedBoost,  // A coin will wobble up and down
  "=": Lava, "|": Lava, "v": Lava  
};
...
SpeedBoost.prototype.type = "speedUp";
...
SpeedBoost.prototype.act = function(step) {
  this.wobble += step * wobSpeedup;
  var wobPos = Math.sin(this.wobble) * wobDist;
  var wobPosup = Math.cos(this.wobble) * wobDist;
  this.pos = this.basePos.plus(new Vector(0, wobPos, wobPosup));
};
...
Flower.prototype.act = function(step) {
  this.wobble += step * flowerspeed;
  var flowerPos = Math.sin(this.wobble) * flowerDist;
  var flowerPosup = Math.cos(this.wobble) * flowerDist;
  this.pos = this.basePos.plus(new Vector(0, flowerPos, flowerPosup));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle == "lava") 
  {
    ...
    else if (obstacle == "tele") 
  {
	  console.log("Player advances forward! Yippie!");
	  this.pos = new Vector(68,14);
  }
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "speedUp" || type == "flower") {
